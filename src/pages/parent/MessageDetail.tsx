...
import { ArrowLeftIcon, PaperAirplaneIcon, UserCircleIcon, ClockIcon, CheckCircleIcon } from '@heroicons/react/24/solid';
...
const MessageDetail: React.FC = () => {
  ...
  const handleReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setReplyText('');
      navigate('/dashboard/parent/messages');
    } catch (error) {
      console.error(error);
    } finally {
      setSending(false);
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <Card>
              ...
              <div className="flex gap-3">
                <Button
                  variant="secondary"
                  onClick={() => navigate(`/dashboard/parent/messages/new?recipient=${message.sender?.id}`)}
                  className="flex-1"
                >
                  Nouveau message
                </Button>
                <Button
                  ...
                  onClick={() => {
                    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce message ?')) {
                      messagesAPI.deleteMessage(parseInt(id || '0'));
                      navigate('/dashboard/parent/messages');
                    }
                  }}
                  className="flex-1"
